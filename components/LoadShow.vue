<template>
    <div class="flex flex-col">
        <img :src="'https://www.themoviedb.org/t/p/original/' + show.backdrop_path" :alt="show.title + ' poster'" class="object-cover w-screen h-auto lg:h-[500px] xl:self-center">
        <div class="flex flex-row p-4 xl:justify-center">
            <div>
                <img v-if="show.poster_path" :src="'https://image.tmdb.org/t/p/w500/' + show.poster_path"
                    :alt="show.title + ' poster'" class="hidden xl:block object-cover w-[200px] h-[300px] rounded-xl shadow-2xl" />
            </div>
            <div class="lg:p-5 xl:mr-32 xl:w-1/5">
                ...
                <p><span class="font-[ibmplexmono] bg-[#e0e0e0] p-1 rounded-lg">{{ show.first_air_date }}</span> - <span class="font-[ibmplexmono] bg-[#e0e0e0] p-1 rounded-lg">{{ show.last_air_date }}</span></p>
                <p class="font-[inter] pt-2"><span class="font-[ibmplexmono] bg-[#e0e0e0] p-1 rounded-lg">{{ show.number_of_seasons }}</span> Seasons, <span class="font-[ibmplexmono] bg-[#e0e0e0] p-1 rounded-lg">{{ show.number_of_episodes }}</span> Episodes</p>
        ...
        <div class="p-4 lg:p-10">
            <details>
                ...
                <ul>
                    <li v-for="actor in actors" :key="actor.id" class="font-[inter] p-2">
                        <NuxtLink :href="'/crew/' + actor.id" class="flex flex-row p-2 lg:p-4 rounded-lg hover:bg-gray-300 ease-in-out duration-300">
                ...
                <ul>
                    <li v-for="member in crew.crew" :key="member.id" class="font-[inter] p-2">
                        <NuxtLink :href="'/crew/' + member.id" class="flex flex-row p-2 lg:p-4 rounded-lg hover:bg-gray-300 ease-in-out duration-300">
                            ...
                            <img v-else src="@/assets/images/placeholders/person.png" alt="No picture available" class="w-[80px] h-[120px] rounded-xl shadow-2xl xl:ml-5">
                            <div class="p-3">
            ...
            <details>
                ...
                <div class="p-2 font-[inter]">
                    ...
                    <ul class="list-disc px-6 py-3">
                        <li v-for="season in show.seasons" :key="season.id">{{ season.name }} - {{ season.air_date }} - {{ season.episode_count }} episodes</li>
                    </ul>
